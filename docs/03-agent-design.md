...
- 根据建立信息、对白长度、动作复杂度、剧情转折和情绪兑现为每个片段规划一个总时长，并让整批总和接近项目目标；不得按平均值机械切片。
- 每片段包含 2–4 个内部镜头；不得因为普通景别变化、对白轮次或单个反应动作创建新的片段。
- 内部镜头只按剧情顺序描述，不预测各 beat 秒数，不输出时间区间或时间戳。
...
- 片段表。
- 含片段总时长、时长依据、有序镜头语言、动作、对白 ID 和音效提示的 Shot Card。
